...
const dbURI =
  "mongodb+srv://clashonkavin:test1234@rocket.j9vwrao.mongodb.net/?retryWrites=true&w=majority";
...
app.post("/login", async (req, res) => {
  const currInstancefromDB = await Profile.findOne({
...
app.get("/connections-data", async (req, res) => {
  await Connection.find().then((data) => res.send(JSON.stringify(data)));
});
...
app.post("/connection-response", async (req, res) => {
...
app.get("/feed/:id", async (req, res) => {
  await QuizDB.find({ creatorsId: req.params.id }).then((data) =>
...
app.get("/history", async (req, res) => {
  ...
  res.send({
    ...
    email: req.session.email,
    id: req.session.userid,
    quizAttempted: req.session.quizAttempted,
  });
});
...
app.post("/quizes/:id", async (req, res) => {
  ...
  await Profile.findOne({ _id: req.session.userid })
    .then((data) => {
      historyarray = data.quizAttempted;
      username = data.name;
    })
    .then(() => historyarray.push([qName, qid, score, total]))
    .catch((err) => console.log(err));
  await Profile.updateOne(
    { _id: req.session.userid },
    { $set: { quizAttempted: historyarray } }
  )
    .then(console.log("profileupdate"))
    .catch((err) => console.log(err));
  await QuizDB.findOne({ _id: qid })
    .then((data) => {
  ...
  await QuizDB.updateOne(
    { _id: qid },
    { $set: { numberoftimestaken: numbertaken, takenby: usersAttempted } }
  )
    .then(console.log("quizupdate"))
    .catch((err) => console.log(err));
});

...
const QuizDetails = () => {
  ...
  const getdata = async () => {
    const response = await axios.get("http://localhost:5000/quizes/" + id, {
      withCredentials: true,
    });
  ...
  return (
    <div>
      ...
      <div className="profilebox">
        ...
        {takenby && attemptCount > 0 && <h4>People who took this quiz :</h4>}
        {takenby &&
          takenby
            ...
            .map((record, recordIndex) => (
              <h4 key={record[0]}>
                <Link to={`/users/${record[0]}`}>{record[1]}</Link>
              </h4>
            ))}
        <p>QuizId: {quizid}</p>

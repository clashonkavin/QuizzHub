...
const QuizDisplay = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/quizeswithuserdetails/${quizid}`, {
  ...
  const handleSubmit = () => {
    if (!submitted) {
      ...
      axios
        .post(
          `http://localhost:5000/quizes/${quizid}`,
          { score: temp, qName, total: quizData.length },
          { withCredentials: true }
        )
        ...
        <div className="profilebox">
          ...
          {numbertaken > 0 &&
            prevAttempts.map((record, recordIndex) => (
              <p key={`${record[0]}-${recordIndex}`}>
                <Link to={`/users/${record[0]}`}>{record[1]}</Link> with a score
                of {record[2]}
              </p>
            ))}
          {numbertaken == 0 && <p>No Record</p>}
          <h3>This Quiz is created by you</h3>
        </div>
      )}
      {submitted && (
        <h2 className="profilebox">
          Submitted, You scored {Score} out of {quizData.length}{" "}
        </h2>
      )}
      {!isOwner && !submitted && (
        <div className="profilebox takequiz">
          <h2>{qName} Quiz</h2>
                  ...
                  <div className="takeoption" key={optionIndex}>
                    <label>
                      <input
                        ...
                        value={option}
                        checked={
                          chosenOptions[questionIndex] === optionIndex + 1
                        }
                        onChange={(event) =>
                          handleOptionChange(questionIndex, event, optionIndex)
                        }
                      />
                      {option}
                    </label>
